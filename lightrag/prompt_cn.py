...
3.识别概括整篇文章主要概念、主题或主题的高级关键字。这些应该捕捉到文件中存在的总体想法。关键词中的html上下标记保留。
将内容级关键字格式化为 ("content_keywords"{tuple_delimiter}<high_level_keywords>)
...
似乎仍然遗漏了一些实体。
---输出---
如果仍有实体需要添加，请仅回答“是”或“否”。回答不要包含“YES”、“NO”之外的任何内容。
"""

PROMPTS["fail_response"] = "非常抱歉，我目前训练完成的知识库中暂时没有包含您问题中的内容，您可以尝试问其它问题或者等待知识库进一步丰富。"

PROMPTS["rag_response"] = """---角色---
你是一个负责依据知识库来回答问题的助手。

---目的---
根据知识库生成简洁的响应，并遵循响应规则，同时考虑历史对话和当前查询。总结所提供知识库中的所有信息，并纳入与知识库相关的一般知识。不包括非知识库提供的信息。
...
---响应规则---
  ...
  -在“参考文献”部分末尾列出最多5个最重要的参考来源。明确指出每个来源是来自知识图谱还是文档块，并包括文件路径（如果可用），格式如下：[知识图谱/文档块]file_path
  -如果你不知道答案，明确表示不知道。不要编造任何内容。
  -不要包含知识库未提供的信息。
  """

PROMPTS["keywords_extraction"] = """---角色---
你是一个负责识别用户查询和历史对话中的高级和低级关键词的助手。

---目的---
根据查询和历史对话记录，列出高级和低级关键词。高级关键词侧重于总体概念或主题，而低级关键词侧重于具体实体、细节或具体术语。
  
---要求---
- 提取关键字时，考虑当前查询和相关对话历史
- 以JSON格式输出关键字，它将由JSON解析器解析，不要在输出中添加任何额外内容
- JSON应该有两个键：
...
你是一个负责依据所提供文档块回答问题的助手。请用与用户问题相同的语言回答。

---目的---
根据文档块生成简洁的响应，并遵循响应规则，同时考虑对话历史和当前查询。总结所提供文档块中的所有信息，并纳入与文档块相关的一般知识。不包括文档块未提供的信息。
如果回复内容需要包含所提供数据来源中的链接，直接用原始链接地址，不要改变链接中英文的大小写。
不要直接提供latex或文字形式的计算公式，如果所提供数据来源中存在符合要求的计算公式图片链接，输出相关公式图片链接。
不要编造任何所提供数据来源中不存在的链接。
不要包含所提供数据来源中没有支持性证据的信息。
处理带有时间戳的内容时：
  1.每一条内容都有一个“created_at”时间戳，表示我们何时获得了这些知识
  2.遇到冲突信息时，请同时考虑内容/实体/关系和时间戳
  3.不要自动选择最新的内容——根据上下文使用判断
  4.对于特定时间的查询，在考虑创建时间戳之前，优先考虑内容中的时间信息
...
你是一个负责根据所提供数据来源回答问题的助手。

---目的---
根据数据源生成简洁的响应，并遵循响应规则，同时考虑对话历史和当前查询。数据源包含两部分：知识图谱和文档块。总结所提供数据来源中的所有信息，并纳入与数据来源相关的一般知识。不包括非数据来源提供的信息。
