...
输出格式：
"YES"或者"NO"<|#|>关键词<|#|>分析内容<|#|>拟出的问题1<|:|>拟出的问题2<|:|>拟出的问题3<|COMPLETE|>
...
################
输出:
NO<|#|>苹果,山楂制品,展青霉素,测定<|#|>问题中未明确"展青霉素测定"所依据的标准类型、标准编号或标准名称，也未说明适用的行业或地区范围。此外，"测定"一词过于宽泛，未明确是检测方法、限量标准还是其他相关内容。<|#|>《食品安全国家标准 食品中真菌毒素限量》（GB 2761-2017）中关于"苹果"和"山楂制品"中"展青霉素"的限量标准是什么？<|:|>《食品安全国家标准 食品中污染物限量》（GB 2762-2017）是否包含"苹果"和"山楂制品"中"展青霉素"的测定方法？<|:|>《进出口食品安全管理办法》中关于"苹果"和"山楂制品"中"展青霉素"的检测要求是什么？<|:|>《食品中展青霉素的测定 高效液相色谱法》（GB 5009.222-2016）是否适用于"苹果"和"山楂制品"？<|:|>《食品中展青霉素的测定 液相色谱-质谱法》（GB 23200.115-2018）在"苹果"和"山楂制品"中的应用范围是什么？<|COMPLETE|>
######################
范例2:
待处理文本:
...
你是一个负责识别用户查询中的关键词、行为、要求和疑问词的助手。
使用{language}作为输出语言。
---目的---
根据查询，列出关键词、行为、要求和疑问词。
---指令---
- 以JSON格式输出关键词。
- JSON应包含两个键：
  - "keywords" 关键词。
...
PROMPTS["custom_keywords_extraction_examples"] = [
    """范例1:
查询: "GB5009.185-2016使用高效液相色谱法测定苹果和山楂制品中展青霉素时，实验的精密度要求是什么？"
################
输出:
{{
...
######################""",
    """范例2:
查询: "《食品中展青霉素的测定》标准中"苹果"和"山楂制品"中"展青霉素"的测定方法是什么？"
################
输出:
{{
...
# PROMPTS["validate_question"] = """---角色---
# 你是中国国家标准、省市地方标准及行业标准等专家，任务是分析并判断给定的文本中的问题是否符合下面的要求，并按要求的格式输出回答。
# ---目的---
# 仔细分析待处理的问题，判断其意思是否明确。判断标准如下：
# 1.问题是否包含足够的信息：问题是否清晰地表达了用户的意图，是否提供了足够的上下文或背景信息，使得大语言模型能够准确理解并给出有针对性的回答。
# 2.问题是否有歧义：问题是否存在多种可能的解释，或者是否使用了模糊的词汇或表达方式，导致大语言模型难以准确理解其意图。
# 3.问题是否具体：问题是否过于宽泛或笼统，缺乏明确的焦点或目标。
# 4.问题是否有疑问词：问题是否存在疑问词。
#
# 判断与优化规则：
# 1.如果问题意思明确，直接输出"YES"。
# 2.如果问题意思不明确，首先输出"NO"，然后根据问题可能的意图，结合提取的专有名词和关键词推测提问者的需求，并给出修改后的问题。修改后的问题应尽量具体、清晰、覆盖可能的意图且意思不重复。
#
# 输出说明：
# 1.如果问题涉及专业领域（如食品、标准化、医学等），请确保修改后的问题符合该领域的常见表达方式。
# 2.如果问题过于模糊或缺乏上下文，针对可能引发歧义的各个方向提出问题。
# 3.修改后的问题应尽量简洁明了，避免引入新的歧义。
# 4.修改后的问题中的专有名词和关键词用""进行标注。
# 5.对待处理的问题进行评分（0-100）分数越高，意思越明确。
# 6.输出的分析内容中只包含不符合判断标准的部分。
#
# 输出格式：
# "YES"或者"NO"<|#|>分数<|#|>专有名词和关键词1,专有名词和关键词2,专有名词和关键词3<|#|>分析内容<|#|>修改后的问题1<|:|>修改后的问题2<|:|>修改后的问题3<|COMPLETE|>
# #############################
# -范例-
# ######################
# 范例1:
# 待处理问题:苹果和山楂制品中展青霉素测定
# ################
# 输出:
# NO<|#|>60<|#|>苹果,山楂制品,展青霉素<|#|>问题中提到的"苹果和山楂制品中展青霉素测定"缺乏具体的上下文信息，如测定的目的、方法、标准等，导致问题不够明确。此外，问题中没有使用疑问词，使得问题的意图不够清晰。<|#|>"苹果"和"山楂制品"中"展青霉素"的测定方法是什么？<|:|>"苹果"和"山楂制品"中"展青霉素"的测定标准有哪些？<|:|>"苹果"和"山楂制品"中"展青霉素"测定的目的是什么？<|COMPLETE|>
# ######################
# 范例2:
# 待处理问题:使用高效液相色谱法测定苹果和山楂制品中展青霉素时，实验的精密度要求是什么？
# ################
# 输出:
# YES<|#|>95<|#|>高效液相色谱法,苹果,山楂制品,展青霉素<|#|>问题意思明确，包含了足够的信息，没有歧义，具体且包含疑问词。<|#|><|COMPLETE|>
# #############################
# 待处理的问题:
# """